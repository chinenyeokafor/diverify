...
class DiVerifyDaemonSigner():
    def __init__(self, key=None):
        self.private_key = key or self.generate_key_pair()
    ...
    def set_and_get_quote(self, user_report):
        user_data = get_user_data(user_report)
        quote = get_quote(user_data)
        return quote
    
    def get_remote_attestation(self, diverify_proof):
        dvp_hash = hashlib.sha256(json.dumps(diverify_proof).encode()).digest()
        dvp_sig = self.private_key.sign(dvp_hash, ec.ECDSA(Prehashed(hashes.SHA256())))
        quote = self.set_and_get_quote(dvp_sig)
        return quote
    # @perf_utils.measure_latency
    def sign(self, payload: bytes, token, diverify_proof: Dict, trust_level, mode=None) -> dict[str, Any]:
        ...
        if mode == "c":
            ...
            diverify_proof["public_key"] = self.private_key.public_key().public_bytes(
                encoding=serialization.Encoding.PEM,
                format=serialization.PublicFormat.SubjectPublicKeyInfo
            ).decode('utf-8')

            quote = self.get_remote_attestation(diverify_proof)
            ...
            if jwt.decode(token, options={"verify_signature": False}).get('aud') == "https://accounts.opk_diverify.test":
                ...
                quote = self.get_remote_attestation(diverify_proof)
                diverify_proof["quote"] = base64.b64encode(quote).decode()
                csr = self.create_csr(email_address, json.dumps(diverify_proof).encode()).sign(self.private_key, hashes.SHA256())
    ...
    def sign_artifact(self, input_: bytes | Hashed) -> tuple[bytes, bytes]:
        ...
        return hashed_input, self.private_key.sign(hashed_input.digest, ec.ECDSA(Prehashed(hashes.SHA256())))
